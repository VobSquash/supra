...
import 'package:dupra/presentation/calculator/widgets/fridge_item_tile.dart';
import 'package:dupra/presentation/calculator/widgets/saved_items_dialog.dart';
...
class _FridgeCalculatorPageState extends State<FridgeCalculatorPage> with SingleTickerProviderStateMixin {
  ...
  static List<FridgeItem> _defaultItemsFor(ItemCategory category) {
    switch (category) {
      ...
      case ItemCategory.others:
        return [const FridgeItem(id: 'squash-ball', name: 'Squash Balls', price: 50)];
    }
  }
  ...
  void initState() {
    ...
    _itemsByCategory = {for (final c in ItemCategory.values) c: _defaultItemsFor(c).map((e) => e.copyWith()).toList()};
    _bootstrap();
  }
  ...
  Future<void> _bootstrap() async {
    ...
    final label = auth.state.maybeWhen(
      authenticated: (s) => s.displayName?.trim().isNotEmpty ?? false ? s.displayName!.trim() : (s.email ?? suffix),
      orElse: () => suffix,
    );
  ...
  void _showSavedItemsDialog() {
    ...
    showDialog<void>(
      context: context,
      builder: (ctx) => SavedItemsDialog(categorizedItems: categorizedItems, timestamp: timestamp, username: username),
    );
  }
                ...
                children: [
                  ...
                  DecoratedBox(
                    decoration: BoxDecoration(
                      color: Color.alphaBlend(scheme.primary.withValues(alpha: 0.06), scheme.surface),
                        ...
                        children: [
                          SingleChildScrollView(
                            ...
                            child: Row(
                              ...
                              children: [
                                FilledButton(onPressed: total > 0 ? _checkout : null, child: Text('Pay R$total')),
                                const SizedBox(width: 10),
                                OutlinedButton(onPressed: _showSavedItemsDialog, child: const Text('View saved')),
                                const SizedBox(width: 10),
