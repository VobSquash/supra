...
class _FakeBookingsFacade implements IBookingsFacade {
  @override
  Future<BookingListDto> loadBookings({required DateTime forDate}) async => BookingListDto(
        bookings: [
          BookingDto.empty().copyWith(
            displayName: 'Court user',
            courtNo: 2,
            bookingDate: DateTime.utc(forDate.year, forDate.month, forDate.day, 14, 30),
            groupBookingId: 0,
          ),
        ],
      );
...
void main() {
  test('onLoadBookings ends in loadingSuccess and keeps selected date', () async {
