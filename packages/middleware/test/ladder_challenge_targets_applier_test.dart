...
LadderItemDTO _player(String guid, int order) => LadderItemDTO.empty().copyWith(
  vobGuid: guid,
  order: order,
  profile: BasicProfileDTO.empty().copyWith(vobGuid: guid),
);

void main() {
  test('applyViewerChallengeTargets clears flags when viewer guid missing', () {
    final items = [_player('a', 1), _player('b', 2).copyWith(canBeChallenged: true)];
    final out = applyViewerChallengeTargets(items: items, viewerVobGuid: null, showLadderBreakdown: true);
  ...
  test('applyViewerChallengeTargets flags the two orders immediately above viewer', () {
    final items = [_player('top', 1), _player('second', 2), _player('third', 3), _player('me', 4), _player('below', 5)];
    final out = applyViewerChallengeTargets(items: items, viewerVobGuid: 'me', showLadderBreakdown: true);
  ...
  test('applyViewerChallengeTargets handles rank #2 viewer with only one above', () {
    final items = [_player('top', 1), _player('me', 2)];
    final out = applyViewerChallengeTargets(items: items, viewerVobGuid: 'me', showLadderBreakdown: true);
  ...
  test('applyViewerChallengeTargets clears targets when viewer is #1', () {
    final items = [_player('me', 1), _player('below', 2)];
    final out = applyViewerChallengeTargets(items: items, viewerVobGuid: 'me', showLadderBreakdown: true);
    expect(out.every((e) => e.canBeChallenged == false), isTrue);
  });
  ...
  test('applyViewerChallengeTargets resolves viewer via profile vobGuid when entry guid empty', () {
    ...
    final items = [_player('above', 1), LadderItemDTO.empty().copyWith(vobGuid: '', order: 2, profile: meProfile)];
    final out = applyViewerChallengeTargets(items: items, viewerVobGuid: 'ME ', showLadderBreakdown: true);
  ...
  test('applyViewerChallengeTargets matches mixed entry/profile viewer row', () {
    ...
    final out = applyViewerChallengeTargets(items: items, viewerVobGuid: 'viewer-guid', showLadderBreakdown: true);
